    ...
    try:
        with mysql.connector.connect(**MYSQL_CONFIG) as conn:
            with conn.cursor() as cursor:
                ...
                query = f"SELECT id, {field_str} FROM data WHERE has = '0' AND `date` <= %s ORDER BY `date` ASC LIMIT 500"
...
def ambil_tmp(fields, date):
    ...
    try:
        with mysql.connector.connect(**MYSQL_CONFIG) as conn:
            with conn.cursor() as cursor:
                field_str = ', '.join(fields)
                query = f"SELECT id, {field_str} FROM tmp WHERE has = '0' AND `date` <= %s ORDER BY `date` ASC LIMIT 500"
                cursor.execute(query, (date,))
                rows = cursor.fetchall()
